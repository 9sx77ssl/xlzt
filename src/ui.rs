...
fn restore_terminal() {
    const SEQ: &[u8] = b"\x1b7\x1b[r\x1b8\r\n\x1b[?25h\x1b[0m";
    unsafe {
...
struct SubmitNonEmpty;

impl ConditionalEventHandler for SubmitNonEmpty {
...
fn term_height() -> Option<u16> {
    unsafe {
        ...
        if libc::ioctl(1, libc::TIOCGWINSZ, &mut ws) == 0 && ws.ws_row > 1 {
            Some(ws.ws_row)
        } else {
            None
        }
    ...
    unsafe {
        ...
        loop {
            ...
            match b[0] {
                ...
                b';' => semi = true,
                b'R' => break,
                c if in_csi && !semi && c.is_ascii_digit() => {
...
type Flash = Arc<Mutex<(&'static str, u8)>>;
...
impl Cat {
    pub fn start() -> Cat {
        let flash: Flash = Arc::new(Mutex::new(("", 0)));
        if !tty() {
            return Cat { stop: Arc::new(AtomicBool::new(true)), handle: None, flash, region: false };
        }
        println!("{ACCENT}^.^{RESET}");
        let _ = io::stdout().flush();
        let (c, h) = match (cursor_row(), term_height()) {
            (Some(c), Some(h)) if c > 1 && c < h => (c, h),
            _ => return Cat { stop: Arc::new(AtomicBool::new(true)), handle: None, flash, region: false },
        };
        print!("\x1b[{c};{h}r\x1b[{c};1H");
        let _ = io::stdout().flush();
        let row = c - 1;
        ...
        let handle = thread::spawn(move || {
            let mut i = 0usize;
            while !flag.load(Ordering::Relaxed) && !QUIET.load(Ordering::Relaxed) {
                ...
                if msg.is_empty() {
                    ...
                    print!("\x1b7\x1b[{row};1H\x1b[2K{ACCENT}{face}{RESET}  {DIM}{msg}{RESET}\x1b8");
                }
                let _ = io::stdout().flush();
                i += 1;
        ...
        Cat { stop, handle: Some(handle), flash, region: true }
    }
...
pub fn ask_title() -> Option<String> {
    ...
    ed.bind_sequence(
        ...
        EventHandler::Conditional(Box::new(SubmitNonEmpty)),
    );
    ed.readline("title › ").ok().map(|s| s.trim().to_string())
}
...
fn clipboard_image() -> Option<(Vec<u8>, String)> {
    clipboard_raw().or_else(clipboard_file)
}

fn clipboard_raw() -> Option<(Vec<u8>, String)> {
...
fn clipboard_text(kind: &str) -> Option<String> {
    let wayland = std::env::var_os("WAYLAND_DISPLAY").is_some();
    let bytes = if wayland {
        capture(&["wl-paste", "-n", "--type", kind])?
    } else {
        capture(&["xclip", "-selection", "clipboard", "-t", kind, "-o"])?
    };
    Some(String::from_utf8_lossy(&bytes).into_owned())
}

fn clipboard_file() -> Option<(Vec<u8>, String)> {
    for kind in ["text/uri-list", "text/plain", "UTF8_STRING"] {
        let Some(text) = clipboard_text(kind) else {
            continue;
        };
        for line in text.lines() {
            ...
            if let Some(img) = read_image_file(&uri_to_path(line)) {
                return Some(img);
            }
        }
    }
...
fn uri_to_path(s: &str) -> String {
    if let Some(rest) = s.strip_prefix("file://") {
        let rest = rest.find('/').map_or(rest, |i| &rest[i..]);
        percent_decode(rest)
    } else {
        s.to_string()
    }
...
fn read_image_file(path: &str) -> Option<(Vec<u8>, String)> {
...
impl ConditionalEventHandler for PasteImage {
    fn handle(&self, _: &Event, _: RepeatCount, _: bool, ctx: &EventContext<'_>) -> Option<Cmd> {
        match clipboard_image() {
            ...
            None => {
                *self.flash.lock().unwrap() = ("no image", 8);
                Some(Cmd::Noop)
            }
        }
    }
...
impl ConditionalEventHandler for PostGuard {
    fn handle(&self, _: &Event, _: RepeatCount, _: bool, ctx: &EventContext<'_>) -> Option<Cmd> {
        if ctx.line().trim().is_empty() {
            *self.flash.lock().unwrap() = ("no body", 8);
            Some(Cmd::Noop)
        } else {
            Some(Cmd::AcceptLine)
        }
...
pub fn body(flash: Flash) -> Option<(String, Vec<(Vec<u8>, String)>)> {
    ...
    ed.bind_sequence(
        KeyEvent::ctrl('d'),
        EventHandler::Conditional(Box::new(PostGuard {
            flash: flash.clone(),
        })),
    );
    ed.bind_sequence(
