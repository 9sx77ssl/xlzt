...
fn clipboard_image() -> Option<(Vec<u8>, String)> {
    ...
    let mime = ["image/png", "image/jpeg", "image/webp", "image/gif"]
        .into_iter()
        .find(|m| avail.contains(m))
        .or_else(|| avail.iter().copied().find(|l| l.starts_with("image/")))?;
...
impl ConditionalEventHandler for PasteImage {
    fn handle(&self, _: &Event, _: RepeatCount, _: bool, _: &EventContext<'_>) -> Option<Cmd> {
        match clipboard_image() {
            Some(img) => {
                self.images.lock().unwrap().push(img);
                Some(Cmd::Insert(1, "[Image]".to_string()))
            }
            None => {
...
impl ConditionalEventHandler for DelImage {
    fn handle(&self, _: &Event, _: RepeatCount, _: bool, ctx: &EventContext<'_>) -> Option<Cmd> {
        ...
        if head.ends_with("[Image]") {
            let idx = head.matches("[Image]").count().saturating_sub(1);
            let mut imgs = self.images.lock().unwrap();
            if idx < imgs.len() {
                imgs.remove(idx);
            }
            Some(Cmd::Kill(Movement::BackwardChar(7)))
        } else {
            None
        }
    }
...
pub fn success(msg: &str, url: &str) {
    let (msg, url) = (msg.to_string(), url.to_string());
    if styled() {
