...
async fn run_thread(forum: Option<u64>) -> i32 {
    ...
    let cat = ui::Cat::start();
    let flash = cat.flash();
    let title = match ui::ask_title() {
        Some(t) => t,
        None => {
            cat.stop();
            return 1;
    ...
    let (mut body, images) = match ui::body(flash) {
        Some((b, imgs)) if !b.trim().is_empty() => (b, imgs),
        Some(_) => {
            cat.stop();
    ...
    let n = body.matches(ui::IMG).count().min(images.len());
    if n > 0 {
        let imgs: Vec<_> = images.into_iter().take(n).collect();
        let spin = ui::Spin::start("uploading");
        let res = cdn::upload(imgs).await;
        spin.stop();
        match res {
            Ok(urls) => body = embed_urls(&body, &urls),
            Err(e) => {
                ui::fail(&e);
                return 1;
            }
...
fn embed_urls(body: &str, urls: &[String]) -> String {
    let mut out = String::with_capacity(body.len());
    let mut rest = body;
    let mut i = 0;
    while let Some(pos) = rest.find(ui::IMG) {
        out.push_str(&rest[..pos]);
        rest = &rest[pos + ui::IMG.len()..];
        if i < urls.len() {
...
fn run_settings() -> i32 {
    ...
    let tip = match cfg.token() {
        Some(t) => {
            let n = t.chars().count();
            if n >= 6 {
                format!("…{}", t.chars().skip(n - 6).collect::<String>())
            } else if n > 0 {
                "set".to_string()
            } else {
                "empty".to_string()
