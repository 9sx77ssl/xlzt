...
`Ctrl+V` in the body pastes a clipboard image as `[Image]` (only PNG/JPG/GIF/WEBP are accepted; other types are ignored); on send each one is uploaded to `y7v.lol` and the placeholder becomes a link in place. Needs `xclip` (X11) or `wl-clipboard` (Wayland).
