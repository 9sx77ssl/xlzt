...
`Ctrl+V` in the body pastes a clipboard image as a single image-icon block (only PNG/JPG/GIF/WEBP are accepted; other types are ignored); one Backspace removes the whole block. On send each one is uploaded to `y7v.lol` and the block becomes a link in place. Needs `xclip` (X11) or `wl-clipboard` (Wayland).
